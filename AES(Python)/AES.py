...
def convertNumToBinary(x): # coverts decimal to binary
    y = ""
    for i in range(len(x)):
        val = ord(x[i]) #converts char to ascii value
        k = str(binary_repr(val, width=8))
        y=y+k
    return y
...
if __name__ == "__main__":
    ...
    if len(key) != 2:
        ...
        exit(0)
    key = convertNumToBinary(key)
    print(key)
    keylist = keyExpansion(key)
    print(keylist)
    if len(x) % 2 != 0:
        x += '#'  # filler - #
    x = convertNumToBinary(x)
    print(x)
    x = list(map(int, x))
    print(x)
    i = 0
